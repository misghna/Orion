...
@Injectable()
export class ExporterQuoteService {
  baseUrl; 
  ...
  getAll(){
    var url = this.baseUrl + 'api/user/exporterQuote'
    return this.http.get(url,[{ withCredentials: true }])
      .map(res => res.json());
  }

  get(type){
    var url = this.baseUrl + 'api/user/exporterQuote/' + type
    return this.http.get(url,[{ withCredentials: true }])
      .map(res => res.json());
  }
  ...
  update(body){
    let headerContent = new Headers();
    headerContent.append("Content-Type", "application/json");
    return this.http.put(this.baseUrl + '/api/user/exporterQuote', body, { headers: headerContent })
            .map(res => res.json());
  }
