...
public class ItemDAOImpl implements ItemDAO {
	...
	public List<String> getLatestNameBrandList() {
		String sql = "select distinct concat('(', i.id, ') ',  i.name, ' [' , i.brand ,']') as fullname from items i"
				+ " where revision  = (select distinct revision from items order by revision desc limit 1) order by fullname asc";
